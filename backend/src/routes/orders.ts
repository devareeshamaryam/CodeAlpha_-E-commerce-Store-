...
// GET /api/orders - Get all orders (admin only, use supabaseAdmin to bypass RLS)
...
// POST /api/orders - anyone can create (use supabaseAdmin to bypass RLS safely on backend)
router.post("/", async (req: Request, res: Response) => {
  try {
    ...
    const total_amount = items.reduce((sum: number, item: OrderItem) => sum + item.price * item.quantity, 0);
    const newOrder: Partial<Order> = {
      customer_name, customer_email, customer_phone,
      shipping_address, items, total_amount, status: "pending",
    };
...
router.patch("/:id/status", async (req: Request, res: Response) => {
  try {
    ...
    const { data, error } = await supabaseAdmin
      .from("orders").update({ status, updated_at: new Date().toISOString() })
      .eq("id", id).select().single();
    if (error || !data) {
      return res.status(404).json({ success: false, error: "Order not found or failed to update" });
    }
...
router.delete("/:id", async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { data, error } = await supabaseAdmin.from("orders").delete().eq("id", id).select().single();
    if (error || !data) {
      return res.status(404).json({ success: false, error: "Order not found" });
    }
